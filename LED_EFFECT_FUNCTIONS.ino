...
void rainbow_fade() {                         //-m2-FADE ALL LEDS THROUGH HSV RAINBOW
  ...
  for (int idex = start_led ; idex < LED_COUNT; idex++ ) {
...
void random_burst() {                         //-m4-RANDOM INDEX/COLOR
  idex = random(start_led, LED_COUNT);
  ihue = random(0, 255);
  leds[idex] = CHSV(ihue, thissat, 255);
//  LEDS.show();
//  delay(thisdelay);
...
void ems_lightsALL() {                  //-m8-EMERGENCY LIGHTS (TWO COLOR SOLID)
  ...
  int idexR = idex;
  int idexB = antipodal_index(idexR);
  if (idexB < start_led) idexB = idexB + start_led;
  int thathue = (thishue + 160) % 255;
...
void fade_vertical() {                    //-m12-FADE 'UP' THE LOOP
  ...
  if (idexB < start_led) idexB = start_led;
  ibright = ibright + 10;
  if (ibright > 255) {
    ibright = 0;
  }
...
void random_march() {                   //-m14-RANDOM MARCH CCW
  ...
  for (int idex = start_led + 1; idex < LED_COUNT ; idex++ ) {
